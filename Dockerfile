...
#Copy the current directory contentes into the container at /var/www/thml
COPY . /var/www/html

#Install any dependencies your PHP application may need
#For example, if you're using Composer for depenency management:
#Run apt-get update && apt-get install -y /
#       git \
...
#LEAVE BELOW 2 LINES UNTIL READY TO DEPLOY (MAYBE)
